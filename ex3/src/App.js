...
function TodoApp() {
  ...
  const onClickAddImportance = (id) => {
    ...
    // setTodos(updatedTodos);  // XXX => set함수는 한 번만 사용 가능
    reorder(updatedTodos);
  }
  ...
  return (
    <div className='flex-wrapper'>
      <header><h1>오늘의 할 일! 2024.08.21</h1></header>
      
      <div className='content'>
        <h2>ToDo-List</h2>
        <ul className='list-box'>{todosList}</ul>
        <div className='add-list-box'>
          <input className='input-text' value={inputText} onChange={onChangeHandler}/>
          <button className='add-btn' onClick={onClickHandler}>추가하기</button>
        </div>
      </div>
